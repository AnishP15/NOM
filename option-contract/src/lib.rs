use cosmwasm_std::{
    to_binary, Binary, Deps, DepsMut, Env, MessageInfo, Response, StdError, StdResult, CosmosMsg, WasmMsg, BankMsg,
 };
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::Transfer { contract_id, new_owner } => {
        ...
        ExecuteMsg::Bid { contract_id, bid_amount } => {
            let mut state = State::from_storage(&mut deps.storage)?;
            let mut contract = state
                .contracts
                .get_mut(&contract_id)
